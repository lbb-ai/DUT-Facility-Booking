...
from utils.email_service import send_booking_approved, send_booking_rejected
...
def approve_booking(booking_id):
    ...
    conflicts = Booking.check_conflict(booking.facility_id, booking.booking_date,
                    booking.start_time, booking.end_time, exclude_id=booking.id)
    if conflicts:
        conflict_statuses = list(set(c.status for c in conflicts))
        flash(f'Cannot approve: time slot conflict with an existing {" / ".join(conflict_statuses)} booking.', 'danger')
    ...
    db.session.add(Notification(user_id=booking.user_id,
        ...
        type='success', booking_id=booking.id))
    db.session.commit()

    try: send_booking_approved(booking)
...
def reject_booking(booking_id):
    ...
    db.session.add(Notification(user_id=booking.user_id,
        message=f'Your booking "{booking.title}" for {booking.facility.name} '
                f'on {booking.booking_date} has been REJECTED. Reason: {booking.admin_notes}',
        type='danger', booking_id=booking.id))
    db.session.commit()

    try: send_booking_rejected(booking)
    except Exception: pass
